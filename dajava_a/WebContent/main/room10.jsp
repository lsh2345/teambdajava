	...
	<meta charset="UTF-8">
		...
		$(function(){
			...
			var supply = text.replace("1","TV, ").replace("2","식탁, ").replace("3","냉장고, ").replaceAll("4","드라이기, ").replace("5","취사도구, ").replace("6","전기밥솥, ").replace("7","인터넷, ").replaceAll("8","욕실용품, ").replace("9","옷장, ").replaceAll("10","에어컨 ");
			 ...
			 function changePic(idx) {
				...
				switch(num){
				case 1:
					$(".header").css({"background":"url(<%=request.getContextPath()%>/filefolder/ry-living.jpg)"});
					$(".header").css({"background-size":"cover"});
					break;
				case 2:
					$(".header").css({"background":"url(<%=request.getContextPath()%>/filefolder/ry.png)"});
					$(".header").css({"background-size":"cover"});
					break;
				case 3:
					$(".header").css({"background":"url(<%=request.getContextPath()%>/filefolder/ry-kitchen.jpg)"});
					$(".header").css({"background-size":"cover"});
					break;
				}				
		...
		<style>
			.header {
			  width: 100%;
			  height: 100vh;
			  background:url('<%=request.getContextPath()%>/filefolder/ry-living.jpg');
